...
class Store {
 ...
 public:
    ...
    void multi_get(const GSRowKeyPredicateEntry *const*predicateList,
                   size_t predicateCount, GSContainerRowEntry **entryList,
                   size_t *containerCount, int **colNumList, GSType ***typeList,
                   int **orderFromInput);
 ...
 private:
    explicit Store(GSGridStore *store);
    void freeMemoryMultiGet(int **colNumList, GSType ***typeList, int length,
                            int **orderFromInput);
    bool setMultiContainerNumList(
